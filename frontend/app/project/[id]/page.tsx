...
// Next.js 15 requiere que desempaquetemos params con `use()`
    ...
    <main className="min-h-screen p-4 md:p-8 relative">
      <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="bg-slate-900/50 backdrop-blur-xl p-6 rounded-2xl border border-slate-700/50 shadow-lg flex items-center justify-between">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-slate-100 flex items-center gap-3">
                Tareas del Proyecto 
                <span className="px-3 py-1 bg-cyan-500/20 text-cyan-400 rounded-lg text-lg border border-cyan-500/30">#{projectId}</span>
              </h1>
            </div>
          </div>
        </div>
        ...
        <section className="bg-slate-900/40 backdrop-blur-lg p-6 rounded-2xl border border-slate-700/50 shadow-lg relative overflow-hidden">
          ...
          <form onSubmit={handleCreateTask} className="flex flex-col md:flex-row gap-4 relative">
            <input 
              type="text" 
            ...
            <input 
              ...
              className="w-full md:w-48 p-3.5 bg-slate-950/50 border border-slate-700/80 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-slate-100 placeholder-slate-500 transition-all" 
            />
            <button 
              type="submit" 
              className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3.5 px-6 rounded-xl transition-all shadow-lg shadow-cyan-500/25 active:scale-[0.98] whitespace-nowrap"
            >
              Crear Tarea
            </button>
          </form>
                ...
                <Link href={`/task/${task.id}`} key={task.id} className="block group">
                  <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-xl border border-slate-700/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 group-hover:bg-slate-800/80 group-hover:border-cyan-500/50 transition-all cursor-pointer relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 border-slate-700/50 pt-3 sm:pt-0 relative">
                      <span className="bg-amber-500/10 text-amber-500 border border-amber-500/20 text-xs font-bold px-3 py-1.5 rounded-md uppercase tracking-wide flex items-center gap-1">
